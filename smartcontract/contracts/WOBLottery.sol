...
contract wobLottery{
    ...
    */
    ...
    function sendETHtoWinners() private{
        uint256 gatheredWeiTotalToBeSent = (GATHERED_WEI / 100)*95; // 전체 금액의 95퍼센트를 당첨자에게
        uint8 winningNumber = getWinningNumber();
        uint numberOfWinners = PARTICIPANTS[GAME_NUMBER][winningNumber].length;
        // 전송
        for(uint i = 0; i < numberOfWinners; i++){
            PARTICIPANTS[GAME_NUMBER][winningNumber][i].transfer(gatheredWeiTotalToBeSent/numberOfWinners);
        }
